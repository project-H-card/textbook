...
def template_to_pages(
    ...
    def create_all_page_file(page_divs):
        with open(ALL_TEMPLATE_FILE_PATH, "r", encoding="utf-8") as f:
            template = f.read()
            template = template.replace("{page_divs}", page_divs)
            print(template[300:500])
        with open(os.path.join(RESULT_DIR, "all/index.html"), "w", encoding="utf-8") as result_f:
            result_f.write(template)
...
if __name__ == "__main__":
    ...
    TEMPLATE_FILE_PATH = "fefw_templates/template.html"
    INDEX_TEMPLATE_FILE_PATH = "fefw_templates/index_template.html"
    PAGES_DIV_TEMPLATE_FILE_PATH = "fefw_templates/pages_div_template.html"
    ALL_TEMPLATE_FILE_PATH = "fefw_templates/all_template.html"
    RESULT_DIR = "fefw_pages/"
